...
class ResizeIfTooLarge(Augmentation):
    """
    Scale the shorter edge to the given size, with a limit of `max_size` on the longer edge.
    If `max_size` is reached, then downscale so that the longer edge does not exceed max_size.
    """
    ...
    def __init__(
        self, max_size=sys.maxsize, interp=Image.BILINEAR
    ):
        """
        Args:
            short_edge_length (list[int]): If ``sample_style=="range"``,
...
class RandomResizedCrop(Augmentation):
    ...
    def get_transform(self, image):
        h, w = image.shape[:2]
        i, j, h, w = self.get_crop_size((h, w))
        # assert h >= croph and w >= cropw, "Shape computation in {} has bugs.".format(self)
        # h0 = np.random.randint(h - croph + 1)
        # w0 = np.random.randint(w - cropw + 1)
        return TransformList([CropTransform(j, i, w, h), ResizeTransform(h, w, self.size[0], self.size[1])])
...
class CentralCrop(Augmentation):
    ...
    def __init__(self, crop_size, crop_type: str = "absolute"):
        """
        Args:
            crop_type (str): one of "relative_range", "relative", "absolute", "absolute_range".
            crop_size (tuple[float, float]): two floats, explained below.
    ...
    def get_transform(self, image: np.ndarray) -> TransformList:
        ...
        h, w = image.shape[:2]
        croph, cropw = self.get_crop_size((h, w))
        output_size = (croph, cropw)

        # Add random crop if the image is scaled up.
        x0 = int(w / 2 - cropw / 2)
        y0 = int(h / 2 - croph / 2)
        crop_transform = CropTransform(x0, y0, cropw, croph, input_size[1], input_size[0])
...
class FixedExtent(Augmentation):
    """
    Outputs an image by cropping a random "subrect" of the source image.
    ...
    def get_transform(self, image: np.ndarray, crop: np.ndarray):
        assert crop is not None, "Crop was not properly set as an attribute of AugInput"
        img_h, img_w = image.shape[:2]
        src_rect = crop
        return ExtentTransform(
            src_rect=(src_rect[0], src_rect[1], src_rect[2], src_rect[3]),
